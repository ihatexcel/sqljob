        ...
        try {
            ...
            if (copyMatch) {
                ...
                if (materialize !== 'ephemeral' && cell.name?.trim()) {
                    ...
                    console.log('[chart] branche ephémère → chartConfig columns:', chartConfig?.columns?.length, 'finalQuery (début):', finalQuery?.slice(0, 120))
                    if (chartConfig?.columns?.length) {
                        ...
                        console.log('[chart] executeQueryWithSchema → columnTypes:', JSON.stringify(columnTypes))
