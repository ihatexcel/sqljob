...
import { CellRenderer } from './CellRenderer'
        ...
        export class CellBodyRenderer {
            ...
            static renderSqlWithTable(pathExpr, cellIdxExpr, schema) {
                ...
                out += `<div x-show="showSqlEditorVisible(cellItem.cell)" x-effect="safeRenderSqlEditor($el, cellItem.cell, '${defaultSql}', false, 'query', null, null, null, null, '${pathExpr}', '${cellIdxExpr}')"></div>`;
                if (showTextResult) {
            ...
            static renderSqlWithEchart(pathExpr, cellIdxExpr, schema) {
                ...
                return `<div :class="hasCellHeight(cellItem.cell) ? 'flex-1 min-h-0 flex flex-col' : 'flex flex-col gap-2'">
                    <template x-if="showSqlEditorVisible(cellItem.cell)"><div x-effect="safeRenderSqlEditor($el, cellItem.cell, '${defaultSql}', false, 'query', null, null, null, null, '${pathExpr}', '${cellIdxExpr}')"></div></template>
