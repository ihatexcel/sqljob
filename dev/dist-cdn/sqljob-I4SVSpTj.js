import { o as U, a as T, t as AB, b as W, d as EA, s as F, e as V, f as l, u as wA, r as y, h as WA, i as cA, Z as BB } from "./sqljob-BgaIYYfi.js";
const EB = 1.333, wB = 0.3528, QB = 2.8346, uB = 3.7795275591, VB = {
  width: 210,
  height: 297,
  padding: [10, 10, 10, 10]
}, sB = "data:application/pdf;base64,", lB = sB, CB = "Roboto", gB = "data:font/ttf;base64,";
var QA = {}, p = {};
p.byteLength = iB;
p.toByteArray = MB;
p.fromByteArray = RB;
var H = [], j = [], IB = typeof Uint8Array < "u" ? Uint8Array : Array, BA = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
for (var a = 0, FB = BA.length; a < FB; ++a)
  H[a] = BA[a], j[BA.charCodeAt(a)] = a;
j[45] = 62;
j[95] = 63;
function nA(g) {
  var i = g.length;
  if (i % 4 > 0)
    throw new Error("Invalid string. Length must be a multiple of 4");
  var D = g.indexOf("=");
  D === -1 && (D = i);
  var M = D === i ? 0 : 4 - D % 4;
  return [D, M];
}
function iB(g) {
  var i = nA(g), D = i[0], M = i[1];
  return (D + M) * 3 / 4 - M;
}
function DB(g, i, D) {
  return (i + D) * 3 / 4 - D;
}
function MB(g) {
  var i, D = nA(g), M = D[0], R = D[1], L = new IB(DB(g, M, R)), Y = 0, s = R > 0 ? M - 4 : M, b;
  for (b = 0; b < s; b += 4)
    i = j[g.charCodeAt(b)] << 18 | j[g.charCodeAt(b + 1)] << 12 | j[g.charCodeAt(b + 2)] << 6 | j[g.charCodeAt(b + 3)], L[Y++] = i >> 16 & 255, L[Y++] = i >> 8 & 255, L[Y++] = i & 255;
  return R === 2 && (i = j[g.charCodeAt(b)] << 2 | j[g.charCodeAt(b + 1)] >> 4, L[Y++] = i & 255), R === 1 && (i = j[g.charCodeAt(b)] << 10 | j[g.charCodeAt(b + 1)] << 4 | j[g.charCodeAt(b + 2)] >> 2, L[Y++] = i >> 8 & 255, L[Y++] = i & 255), L;
}
function YB(g) {
  return H[g >> 18 & 63] + H[g >> 12 & 63] + H[g >> 6 & 63] + H[g & 63];
}
function LB(g, i, D) {
  for (var M, R = [], L = i; L < D; L += 3)
    M = (g[L] << 16 & 16711680) + (g[L + 1] << 8 & 65280) + (g[L + 2] & 255), R.push(YB(M));
  return R.join("");
}
function RB(g) {
  for (var i, D = g.length, M = D % 3, R = [], L = 16383, Y = 0, s = D - M; Y < s; Y += L)
    R.push(LB(g, Y, Y + L > s ? s : Y + L));
  return M === 1 ? (i = g[D - 1], R.push(
    H[i >> 2] + H[i << 4 & 63] + "=="
  )) : M === 2 && (i = (g[D - 2] << 8) + g[D - 1], R.push(
    H[i >> 10] + H[i >> 4 & 63] + H[i << 2 & 63] + "="
  )), R.join("");
}
var sA = {};
/*! ieee754. BSD-3-Clause License. Feross Aboukhadijeh <https://feross.org/opensource> */
sA.read = function(g, i, D, M, R) {
  var L, Y, s = R * 8 - M - 1, b = (1 << s) - 1, P = b >> 1, n = -7, e = D ? R - 1 : 0, S = D ? -1 : 1, h = g[i + e];
  for (e += S, L = h & (1 << -n) - 1, h >>= -n, n += s; n > 0; L = L * 256 + g[i + e], e += S, n -= 8)
    ;
  for (Y = L & (1 << -n) - 1, L >>= -n, n += M; n > 0; Y = Y * 256 + g[i + e], e += S, n -= 8)
    ;
  if (L === 0)
    L = 1 - P;
  else {
    if (L === b)
      return Y ? NaN : (h ? -1 : 1) * (1 / 0);
    Y = Y + Math.pow(2, M), L = L - P;
  }
  return (h ? -1 : 1) * Y * Math.pow(2, L - M);
};
sA.write = function(g, i, D, M, R, L) {
  var Y, s, b, P = L * 8 - R - 1, n = (1 << P) - 1, e = n >> 1, S = R === 23 ? Math.pow(2, -24) - Math.pow(2, -77) : 0, h = M ? 0 : L - 1, Z = M ? 1 : -1, k = i < 0 || i === 0 && 1 / i < 0 ? 1 : 0;
  for (i = Math.abs(i), isNaN(i) || i === 1 / 0 ? (s = isNaN(i) ? 1 : 0, Y = n) : (Y = Math.floor(Math.log(i) / Math.LN2), i * (b = Math.pow(2, -Y)) < 1 && (Y--, b *= 2), Y + e >= 1 ? i += S / b : i += S * Math.pow(2, 1 - e), i * b >= 2 && (Y++, b /= 2), Y + e >= n ? (s = 0, Y = n) : Y + e >= 1 ? (s = (i * b - 1) * Math.pow(2, R), Y = Y + e) : (s = i * Math.pow(2, e - 1) * Math.pow(2, R), Y = 0)); R >= 8; g[D + h] = s & 255, h += Z, s /= 256, R -= 8)
    ;
  for (Y = Y << R | s, P += R; P > 0; g[D + h] = Y & 255, h += Z, Y /= 256, P -= 8)
    ;
  g[D + h - Z] |= k * 128;
};
/*!
 * The buffer module from node.js, for the browser.
 *
 * @author   Feross Aboukhadijeh <https://feross.org>
 * @license  MIT
 */
(function(g) {
  const i = p, D = sA, M = typeof Symbol == "function" && typeof Symbol.for == "function" ? Symbol.for("nodejs.util.inspect.custom") : null;
  g.Buffer = s, g.SlowBuffer = fA, g.INSPECT_MAX_BYTES = 50;
  const R = 2147483647;
  g.kMaxLength = R, s.TYPED_ARRAY_SUPPORT = L(), !s.TYPED_ARRAY_SUPPORT && typeof console < "u" && typeof console.error == "function" && console.error(
    "This browser lacks typed array (Uint8Array) support which is required by `buffer` v5.x. Use `buffer` v4.x if you require old browser support."
  );
  function L() {
    try {
      const E = new Uint8Array(1), A = { foo: function() {
        return 42;
      } };
      return Object.setPrototypeOf(A, Uint8Array.prototype), Object.setPrototypeOf(E, A), E.foo() === 42;
    } catch {
      return !1;
    }
  }
  Object.defineProperty(s.prototype, "parent", {
    enumerable: !0,
    get: function() {
      if (s.isBuffer(this))
        return this.buffer;
    }
  }), Object.defineProperty(s.prototype, "offset", {
    enumerable: !0,
    get: function() {
      if (s.isBuffer(this))
        return this.byteOffset;
    }
  });
  function Y(E) {
    if (E > R)
      throw new RangeError('The value "' + E + '" is invalid for option "size"');
    const A = new Uint8Array(E);
    return Object.setPrototypeOf(A, s.prototype), A;
  }
  function s(E, A, B) {
    if (typeof E == "number") {
      if (typeof A == "string")
        throw new TypeError(
          'The "string" argument must be of type string. Received type number'
        );
      return e(E);
    }
    return b(E, A, B);
  }
  s.poolSize = 8192;
  function b(E, A, B) {
    if (typeof E == "string")
      return S(E, A);
    if (ArrayBuffer.isView(E))
      return Z(E);
    if (E == null)
      throw new TypeError(
        "The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type " + typeof E
      );
    if (v(E, ArrayBuffer) || E && v(E.buffer, ArrayBuffer) || typeof SharedArrayBuffer < "u" && (v(E, SharedArrayBuffer) || E && v(E.buffer, SharedArrayBuffer)))
      return k(E, A, B);
    if (typeof E == "number")
      throw new TypeError(
        'The "value" argument must not be of type number. Received type number'
      );
    const w = E.valueOf && E.valueOf();
    if (w != null && w !== E)
      return s.from(w, A, B);
    const Q = SA(E);
    if (Q) return Q;
    if (typeof Symbol < "u" && Symbol.toPrimitive != null && typeof E[Symbol.toPrimitive] == "function")
      return s.from(E[Symbol.toPrimitive]("string"), A, B);
    throw new TypeError(
      "The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type " + typeof E
    );
  }
  s.from = function(E, A, B) {
    return b(E, A, B);
  }, Object.setPrototypeOf(s.prototype, Uint8Array.prototype), Object.setPrototypeOf(s, Uint8Array);
  function P(E) {
    if (typeof E != "number")
      throw new TypeError('"size" argument must be of type number');
    if (E < 0)
      throw new RangeError('The value "' + E + '" is invalid for option "size"');
  }
  function n(E, A, B) {
    return P(E), E <= 0 ? Y(E) : A !== void 0 ? typeof B == "string" ? Y(E).fill(A, B) : Y(E).fill(A) : Y(E);
  }
  s.alloc = function(E, A, B) {
    return n(E, A, B);
  };
  function e(E) {
    return P(E), Y(E < 0 ? 0 : q(E) | 0);
  }
  s.allocUnsafe = function(E) {
    return e(E);
  }, s.allocUnsafeSlow = function(E) {
    return e(E);
  };
  function S(E, A) {
    if ((typeof A != "string" || A === "") && (A = "utf8"), !s.isEncoding(A))
      throw new TypeError("Unknown encoding: " + A);
    const B = FA(E, A) | 0;
    let w = Y(B);
    const Q = w.write(E, A);
    return Q !== B && (w = w.slice(0, Q)), w;
  }
  function h(E) {
    const A = E.length < 0 ? 0 : q(E.length) | 0, B = Y(A);
    for (let w = 0; w < A; w += 1)
      B[w] = E[w] & 255;
    return B;
  }
  function Z(E) {
    if (v(E, Uint8Array)) {
      const A = new Uint8Array(E);
      return k(A.buffer, A.byteOffset, A.byteLength);
    }
    return h(E);
  }
  function k(E, A, B) {
    if (A < 0 || E.byteLength < A)
      throw new RangeError('"offset" is outside of buffer bounds');
    if (E.byteLength < A + (B || 0))
      throw new RangeError('"length" is outside of buffer bounds');
    let w;
    return A === void 0 && B === void 0 ? w = new Uint8Array(E) : B === void 0 ? w = new Uint8Array(E, A) : w = new Uint8Array(E, A, B), Object.setPrototypeOf(w, s.prototype), w;
  }
  function SA(E) {
    if (s.isBuffer(E)) {
      const A = q(E.length) | 0, B = Y(A);
      return B.length === 0 || E.copy(B, 0, 0, A), B;
    }
    if (E.length !== void 0)
      return typeof E.length != "number" || AA(E.length) ? Y(0) : h(E);
    if (E.type === "Buffer" && Array.isArray(E.data))
      return h(E.data);
  }
  function q(E) {
    if (E >= R)
      throw new RangeError("Attempt to allocate Buffer larger than maximum size: 0x" + R.toString(16) + " bytes");
    return E | 0;
  }
  function fA(E) {
    return +E != E && (E = 0), s.alloc(+E);
  }
  s.isBuffer = function(A) {
    return A != null && A._isBuffer === !0 && A !== s.prototype;
  }, s.compare = function(A, B) {
    if (v(A, Uint8Array) && (A = s.from(A, A.offset, A.byteLength)), v(B, Uint8Array) && (B = s.from(B, B.offset, B.byteLength)), !s.isBuffer(A) || !s.isBuffer(B))
      throw new TypeError(
        'The "buf1", "buf2" arguments must be one of type Buffer or Uint8Array'
      );
    if (A === B) return 0;
    let w = A.length, Q = B.length;
    for (let C = 0, I = Math.min(w, Q); C < I; ++C)
      if (A[C] !== B[C]) {
        w = A[C], Q = B[C];
        break;
      }
    return w < Q ? -1 : Q < w ? 1 : 0;
  }, s.isEncoding = function(A) {
    switch (String(A).toLowerCase()) {
      case "hex":
      case "utf8":
      case "utf-8":
      case "ascii":
      case "latin1":
      case "binary":
      case "base64":
      case "ucs2":
      case "ucs-2":
      case "utf16le":
      case "utf-16le":
        return !0;
      default:
        return !1;
    }
  }, s.concat = function(A, B) {
    if (!Array.isArray(A))
      throw new TypeError('"list" argument must be an Array of Buffers');
    if (A.length === 0)
      return s.alloc(0);
    let w;
    if (B === void 0)
      for (B = 0, w = 0; w < A.length; ++w)
        B += A[w].length;
    const Q = s.allocUnsafe(B);
    let C = 0;
    for (w = 0; w < A.length; ++w) {
      let I = A[w];
      if (v(I, Uint8Array))
        C + I.length > Q.length ? (s.isBuffer(I) || (I = s.from(I)), I.copy(Q, C)) : Uint8Array.prototype.set.call(
          Q,
          I,
          C
        );
      else if (s.isBuffer(I))
        I.copy(Q, C);
      else
        throw new TypeError('"list" argument must be an Array of Buffers');
      C += I.length;
    }
    return Q;
  };
  function FA(E, A) {
    if (s.isBuffer(E))
      return E.length;
    if (ArrayBuffer.isView(E) || v(E, ArrayBuffer))
      return E.byteLength;
    if (typeof E != "string")
      throw new TypeError(
        'The "string" argument must be one of type string, Buffer, or ArrayBuffer. Received type ' + typeof E
      );
    const B = E.length, w = arguments.length > 2 && arguments[2] === !0;
    if (!w && B === 0) return 0;
    let Q = !1;
    for (; ; )
      switch (A) {
        case "ascii":
        case "latin1":
        case "binary":
          return B;
        case "utf8":
        case "utf-8":
          return $(E).length;
        case "ucs2":
        case "ucs-2":
        case "utf16le":
        case "utf-16le":
          return B * 2;
        case "hex":
          return B >>> 1;
        case "base64":
          return tA(E).length;
        default:
          if (Q)
            return w ? -1 : $(E).length;
          A = ("" + A).toLowerCase(), Q = !0;
      }
  }
  s.byteLength = FA;
  function yA(E, A, B) {
    let w = !1;
    if ((A === void 0 || A < 0) && (A = 0), A > this.length || ((B === void 0 || B > this.length) && (B = this.length), B <= 0) || (B >>>= 0, A >>>= 0, B <= A))
      return "";
    for (E || (E = "utf8"); ; )
      switch (E) {
        case "hex":
          return kA(this, A, B);
        case "utf8":
        case "utf-8":
          return MA(this, A, B);
        case "ascii":
          return TA(this, A, B);
        case "latin1":
        case "binary":
          return ZA(this, A, B);
        case "base64":
          return JA(this, A, B);
        case "ucs2":
        case "ucs-2":
        case "utf16le":
        case "utf-16le":
          return dA(this, A, B);
        default:
          if (w) throw new TypeError("Unknown encoding: " + E);
          E = (E + "").toLowerCase(), w = !0;
      }
  }
  s.prototype._isBuffer = !0;
  function u(E, A, B) {
    const w = E[A];
    E[A] = E[B], E[B] = w;
  }
  s.prototype.swap16 = function() {
    const A = this.length;
    if (A % 2 !== 0)
      throw new RangeError("Buffer size must be a multiple of 16-bits");
    for (let B = 0; B < A; B += 2)
      u(this, B, B + 1);
    return this;
  }, s.prototype.swap32 = function() {
    const A = this.length;
    if (A % 4 !== 0)
      throw new RangeError("Buffer size must be a multiple of 32-bits");
    for (let B = 0; B < A; B += 4)
      u(this, B, B + 3), u(this, B + 1, B + 2);
    return this;
  }, s.prototype.swap64 = function() {
    const A = this.length;
    if (A % 8 !== 0)
      throw new RangeError("Buffer size must be a multiple of 64-bits");
    for (let B = 0; B < A; B += 8)
      u(this, B, B + 7), u(this, B + 1, B + 6), u(this, B + 2, B + 5), u(this, B + 3, B + 4);
    return this;
  }, s.prototype.toString = function() {
    const A = this.length;
    return A === 0 ? "" : arguments.length === 0 ? MA(this, 0, A) : yA.apply(this, arguments);
  }, s.prototype.toLocaleString = s.prototype.toString, s.prototype.equals = function(A) {
    if (!s.isBuffer(A)) throw new TypeError("Argument must be a Buffer");
    return this === A ? !0 : s.compare(this, A) === 0;
  }, s.prototype.inspect = function() {
    let A = "";
    const B = g.INSPECT_MAX_BYTES;
    return A = this.toString("hex", 0, B).replace(/(.{2})/g, "$1 ").trim(), this.length > B && (A += " ... "), "<Buffer " + A + ">";
  }, M && (s.prototype[M] = s.prototype.inspect), s.prototype.compare = function(A, B, w, Q, C) {
    if (v(A, Uint8Array) && (A = s.from(A, A.offset, A.byteLength)), !s.isBuffer(A))
      throw new TypeError(
        'The "target" argument must be one of type Buffer or Uint8Array. Received type ' + typeof A
      );
    if (B === void 0 && (B = 0), w === void 0 && (w = A ? A.length : 0), Q === void 0 && (Q = 0), C === void 0 && (C = this.length), B < 0 || w > A.length || Q < 0 || C > this.length)
      throw new RangeError("out of range index");
    if (Q >= C && B >= w)
      return 0;
    if (Q >= C)
      return -1;
    if (B >= w)
      return 1;
    if (B >>>= 0, w >>>= 0, Q >>>= 0, C >>>= 0, this === A) return 0;
    let I = C - Q, x = w - B;
    const r = Math.min(I, x), o = this.slice(Q, C), t = A.slice(B, w);
    for (let G = 0; G < r; ++G)
      if (o[G] !== t[G]) {
        I = o[G], x = t[G];
        break;
      }
    return I < x ? -1 : x < I ? 1 : 0;
  };
  function iA(E, A, B, w, Q) {
    if (E.length === 0) return -1;
    if (typeof B == "string" ? (w = B, B = 0) : B > 2147483647 ? B = 2147483647 : B < -2147483648 && (B = -2147483648), B = +B, AA(B) && (B = Q ? 0 : E.length - 1), B < 0 && (B = E.length + B), B >= E.length) {
      if (Q) return -1;
      B = E.length - 1;
    } else if (B < 0)
      if (Q) B = 0;
      else return -1;
    if (typeof A == "string" && (A = s.from(A, w)), s.isBuffer(A))
      return A.length === 0 ? -1 : DA(E, A, B, w, Q);
    if (typeof A == "number")
      return A = A & 255, typeof Uint8Array.prototype.indexOf == "function" ? Q ? Uint8Array.prototype.indexOf.call(E, A, B) : Uint8Array.prototype.lastIndexOf.call(E, A, B) : DA(E, [A], B, w, Q);
    throw new TypeError("val must be string, number or Buffer");
  }
  function DA(E, A, B, w, Q) {
    let C = 1, I = E.length, x = A.length;
    if (w !== void 0 && (w = String(w).toLowerCase(), w === "ucs2" || w === "ucs-2" || w === "utf16le" || w === "utf-16le")) {
      if (E.length < 2 || A.length < 2)
        return -1;
      C = 2, I /= 2, x /= 2, B /= 2;
    }
    function r(t, G) {
      return C === 1 ? t[G] : t.readUInt16BE(G * C);
    }
    let o;
    if (Q) {
      let t = -1;
      for (o = B; o < I; o++)
        if (r(E, o) === r(A, t === -1 ? 0 : o - t)) {
          if (t === -1 && (t = o), o - t + 1 === x) return t * C;
        } else
          t !== -1 && (o -= o - t), t = -1;
    } else
      for (B + x > I && (B = I - x), o = B; o >= 0; o--) {
        let t = !0;
        for (let G = 0; G < x; G++)
          if (r(E, o + G) !== r(A, G)) {
            t = !1;
            break;
          }
        if (t) return o;
      }
    return -1;
  }
  s.prototype.includes = function(A, B, w) {
    return this.indexOf(A, B, w) !== -1;
  }, s.prototype.indexOf = function(A, B, w) {
    return iA(this, A, B, w, !0);
  }, s.prototype.lastIndexOf = function(A, B, w) {
    return iA(this, A, B, w, !1);
  };
  function uA(E, A, B, w) {
    B = Number(B) || 0;
    const Q = E.length - B;
    w ? (w = Number(w), w > Q && (w = Q)) : w = Q;
    const C = A.length;
    w > C / 2 && (w = C / 2);
    let I;
    for (I = 0; I < w; ++I) {
      const x = parseInt(A.substr(I * 2, 2), 16);
      if (AA(x)) return I;
      E[B + I] = x;
    }
    return I;
  }
  function VA(E, A, B, w) {
    return z($(A, E.length - B), E, B, w);
  }
  function lA(E, A, B, w) {
    return z(XA(A), E, B, w);
  }
  function KA(E, A, B, w) {
    return z(tA(A), E, B, w);
  }
  function NA(E, A, B, w) {
    return z(qA(A, E.length - B), E, B, w);
  }
  s.prototype.write = function(A, B, w, Q) {
    if (B === void 0)
      Q = "utf8", w = this.length, B = 0;
    else if (w === void 0 && typeof B == "string")
      Q = B, w = this.length, B = 0;
    else if (isFinite(B))
      B = B >>> 0, isFinite(w) ? (w = w >>> 0, Q === void 0 && (Q = "utf8")) : (Q = w, w = void 0);
    else
      throw new Error(
        "Buffer.write(string, encoding, offset[, length]) is no longer supported"
      );
    const C = this.length - B;
    if ((w === void 0 || w > C) && (w = C), A.length > 0 && (w < 0 || B < 0) || B > this.length)
      throw new RangeError("Attempt to write outside buffer bounds");
    Q || (Q = "utf8");
    let I = !1;
    for (; ; )
      switch (Q) {
        case "hex":
          return uA(this, A, B, w);
        case "utf8":
        case "utf-8":
          return VA(this, A, B, w);
        case "ascii":
        case "latin1":
        case "binary":
          return lA(this, A, B, w);
        case "base64":
          return KA(this, A, B, w);
        case "ucs2":
        case "ucs-2":
        case "utf16le":
        case "utf-16le":
          return NA(this, A, B, w);
        default:
          if (I) throw new TypeError("Unknown encoding: " + Q);
          Q = ("" + Q).toLowerCase(), I = !0;
      }
  }, s.prototype.toJSON = function() {
    return {
      type: "Buffer",
      data: Array.prototype.slice.call(this._arr || this, 0)
    };
  };
  function JA(E, A, B) {
    return A === 0 && B === E.length ? i.fromByteArray(E) : i.fromByteArray(E.slice(A, B));
  }
  function MA(E, A, B) {
    B = Math.min(E.length, B);
    const w = [];
    let Q = A;
    for (; Q < B; ) {
      const C = E[Q];
      let I = null, x = C > 239 ? 4 : C > 223 ? 3 : C > 191 ? 2 : 1;
      if (Q + x <= B) {
        let r, o, t, G;
        switch (x) {
          case 1:
            C < 128 && (I = C);
            break;
          case 2:
            r = E[Q + 1], (r & 192) === 128 && (G = (C & 31) << 6 | r & 63, G > 127 && (I = G));
            break;
          case 3:
            r = E[Q + 1], o = E[Q + 2], (r & 192) === 128 && (o & 192) === 128 && (G = (C & 15) << 12 | (r & 63) << 6 | o & 63, G > 2047 && (G < 55296 || G > 57343) && (I = G));
            break;
          case 4:
            r = E[Q + 1], o = E[Q + 2], t = E[Q + 3], (r & 192) === 128 && (o & 192) === 128 && (t & 192) === 128 && (G = (C & 15) << 18 | (r & 63) << 12 | (o & 63) << 6 | t & 63, G > 65535 && G < 1114112 && (I = G));
        }
      }
      I === null ? (I = 65533, x = 1) : I > 65535 && (I -= 65536, w.push(I >>> 10 & 1023 | 55296), I = 56320 | I & 1023), w.push(I), Q += x;
    }
    return aA(w);
  }
  const YA = 4096;
  function aA(E) {
    const A = E.length;
    if (A <= YA)
      return String.fromCharCode.apply(String, E);
    let B = "", w = 0;
    for (; w < A; )
      B += String.fromCharCode.apply(
        String,
        E.slice(w, w += YA)
      );
    return B;
  }
  function TA(E, A, B) {
    let w = "";
    B = Math.min(E.length, B);
    for (let Q = A; Q < B; ++Q)
      w += String.fromCharCode(E[Q] & 127);
    return w;
  }
  function ZA(E, A, B) {
    let w = "";
    B = Math.min(E.length, B);
    for (let Q = A; Q < B; ++Q)
      w += String.fromCharCode(E[Q]);
    return w;
  }
  function kA(E, A, B) {
    const w = E.length;
    (!A || A < 0) && (A = 0), (!B || B < 0 || B > w) && (B = w);
    let Q = "";
    for (let C = A; C < B; ++C)
      Q += _A[E[C]];
    return Q;
  }
  function dA(E, A, B) {
    const w = E.slice(A, B);
    let Q = "";
    for (let C = 0; C < w.length - 1; C += 2)
      Q += String.fromCharCode(w[C] + w[C + 1] * 256);
    return Q;
  }
  s.prototype.slice = function(A, B) {
    const w = this.length;
    A = ~~A, B = B === void 0 ? w : ~~B, A < 0 ? (A += w, A < 0 && (A = 0)) : A > w && (A = w), B < 0 ? (B += w, B < 0 && (B = 0)) : B > w && (B = w), B < A && (B = A);
    const Q = this.subarray(A, B);
    return Object.setPrototypeOf(Q, s.prototype), Q;
  };
  function c(E, A, B) {
    if (E % 1 !== 0 || E < 0) throw new RangeError("offset is not uint");
    if (E + A > B) throw new RangeError("Trying to access beyond buffer length");
  }
  s.prototype.readUintLE = s.prototype.readUIntLE = function(A, B, w) {
    A = A >>> 0, B = B >>> 0, w || c(A, B, this.length);
    let Q = this[A], C = 1, I = 0;
    for (; ++I < B && (C *= 256); )
      Q += this[A + I] * C;
    return Q;
  }, s.prototype.readUintBE = s.prototype.readUIntBE = function(A, B, w) {
    A = A >>> 0, B = B >>> 0, w || c(A, B, this.length);
    let Q = this[A + --B], C = 1;
    for (; B > 0 && (C *= 256); )
      Q += this[A + --B] * C;
    return Q;
  }, s.prototype.readUint8 = s.prototype.readUInt8 = function(A, B) {
    return A = A >>> 0, B || c(A, 1, this.length), this[A];
  }, s.prototype.readUint16LE = s.prototype.readUInt16LE = function(A, B) {
    return A = A >>> 0, B || c(A, 2, this.length), this[A] | this[A + 1] << 8;
  }, s.prototype.readUint16BE = s.prototype.readUInt16BE = function(A, B) {
    return A = A >>> 0, B || c(A, 2, this.length), this[A] << 8 | this[A + 1];
  }, s.prototype.readUint32LE = s.prototype.readUInt32LE = function(A, B) {
    return A = A >>> 0, B || c(A, 4, this.length), (this[A] | this[A + 1] << 8 | this[A + 2] << 16) + this[A + 3] * 16777216;
  }, s.prototype.readUint32BE = s.prototype.readUInt32BE = function(A, B) {
    return A = A >>> 0, B || c(A, 4, this.length), this[A] * 16777216 + (this[A + 1] << 16 | this[A + 2] << 8 | this[A + 3]);
  }, s.prototype.readBigUInt64LE = f(function(A) {
    A = A >>> 0, J(A, "offset");
    const B = this[A], w = this[A + 7];
    (B === void 0 || w === void 0) && d(A, this.length - 8);
    const Q = B + this[++A] * 2 ** 8 + this[++A] * 2 ** 16 + this[++A] * 2 ** 24, C = this[++A] + this[++A] * 2 ** 8 + this[++A] * 2 ** 16 + w * 2 ** 24;
    return BigInt(Q) + (BigInt(C) << BigInt(32));
  }), s.prototype.readBigUInt64BE = f(function(A) {
    A = A >>> 0, J(A, "offset");
    const B = this[A], w = this[A + 7];
    (B === void 0 || w === void 0) && d(A, this.length - 8);
    const Q = B * 2 ** 24 + this[++A] * 2 ** 16 + this[++A] * 2 ** 8 + this[++A], C = this[++A] * 2 ** 24 + this[++A] * 2 ** 16 + this[++A] * 2 ** 8 + w;
    return (BigInt(Q) << BigInt(32)) + BigInt(C);
  }), s.prototype.readIntLE = function(A, B, w) {
    A = A >>> 0, B = B >>> 0, w || c(A, B, this.length);
    let Q = this[A], C = 1, I = 0;
    for (; ++I < B && (C *= 256); )
      Q += this[A + I] * C;
    return C *= 128, Q >= C && (Q -= Math.pow(2, 8 * B)), Q;
  }, s.prototype.readIntBE = function(A, B, w) {
    A = A >>> 0, B = B >>> 0, w || c(A, B, this.length);
    let Q = B, C = 1, I = this[A + --Q];
    for (; Q > 0 && (C *= 256); )
      I += this[A + --Q] * C;
    return C *= 128, I >= C && (I -= Math.pow(2, 8 * B)), I;
  }, s.prototype.readInt8 = function(A, B) {
    return A = A >>> 0, B || c(A, 1, this.length), this[A] & 128 ? (255 - this[A] + 1) * -1 : this[A];
  }, s.prototype.readInt16LE = function(A, B) {
    A = A >>> 0, B || c(A, 2, this.length);
    const w = this[A] | this[A + 1] << 8;
    return w & 32768 ? w | 4294901760 : w;
  }, s.prototype.readInt16BE = function(A, B) {
    A = A >>> 0, B || c(A, 2, this.length);
    const w = this[A + 1] | this[A] << 8;
    return w & 32768 ? w | 4294901760 : w;
  }, s.prototype.readInt32LE = function(A, B) {
    return A = A >>> 0, B || c(A, 4, this.length), this[A] | this[A + 1] << 8 | this[A + 2] << 16 | this[A + 3] << 24;
  }, s.prototype.readInt32BE = function(A, B) {
    return A = A >>> 0, B || c(A, 4, this.length), this[A] << 24 | this[A + 1] << 16 | this[A + 2] << 8 | this[A + 3];
  }, s.prototype.readBigInt64LE = f(function(A) {
    A = A >>> 0, J(A, "offset");
    const B = this[A], w = this[A + 7];
    (B === void 0 || w === void 0) && d(A, this.length - 8);
    const Q = this[A + 4] + this[A + 5] * 2 ** 8 + this[A + 6] * 2 ** 16 + (w << 24);
    return (BigInt(Q) << BigInt(32)) + BigInt(B + this[++A] * 2 ** 8 + this[++A] * 2 ** 16 + this[++A] * 2 ** 24);
  }), s.prototype.readBigInt64BE = f(function(A) {
    A = A >>> 0, J(A, "offset");
    const B = this[A], w = this[A + 7];
    (B === void 0 || w === void 0) && d(A, this.length - 8);
    const Q = (B << 24) + // Overflow
    this[++A] * 2 ** 16 + this[++A] * 2 ** 8 + this[++A];
    return (BigInt(Q) << BigInt(32)) + BigInt(this[++A] * 2 ** 24 + this[++A] * 2 ** 16 + this[++A] * 2 ** 8 + w);
  }), s.prototype.readFloatLE = function(A, B) {
    return A = A >>> 0, B || c(A, 4, this.length), D.read(this, A, !0, 23, 4);
  }, s.prototype.readFloatBE = function(A, B) {
    return A = A >>> 0, B || c(A, 4, this.length), D.read(this, A, !1, 23, 4);
  }, s.prototype.readDoubleLE = function(A, B) {
    return A = A >>> 0, B || c(A, 8, this.length), D.read(this, A, !0, 52, 8);
  }, s.prototype.readDoubleBE = function(A, B) {
    return A = A >>> 0, B || c(A, 8, this.length), D.read(this, A, !1, 52, 8);
  };
  function m(E, A, B, w, Q, C) {
    if (!s.isBuffer(E)) throw new TypeError('"buffer" argument must be a Buffer instance');
    if (A > Q || A < C) throw new RangeError('"value" argument is out of bounds');
    if (B + w > E.length) throw new RangeError("Index out of range");
  }
  s.prototype.writeUintLE = s.prototype.writeUIntLE = function(A, B, w, Q) {
    if (A = +A, B = B >>> 0, w = w >>> 0, !Q) {
      const x = Math.pow(2, 8 * w) - 1;
      m(this, A, B, w, x, 0);
    }
    let C = 1, I = 0;
    for (this[B] = A & 255; ++I < w && (C *= 256); )
      this[B + I] = A / C & 255;
    return B + w;
  }, s.prototype.writeUintBE = s.prototype.writeUIntBE = function(A, B, w, Q) {
    if (A = +A, B = B >>> 0, w = w >>> 0, !Q) {
      const x = Math.pow(2, 8 * w) - 1;
      m(this, A, B, w, x, 0);
    }
    let C = w - 1, I = 1;
    for (this[B + C] = A & 255; --C >= 0 && (I *= 256); )
      this[B + C] = A / I & 255;
    return B + w;
  }, s.prototype.writeUint8 = s.prototype.writeUInt8 = function(A, B, w) {
    return A = +A, B = B >>> 0, w || m(this, A, B, 1, 255, 0), this[B] = A & 255, B + 1;
  }, s.prototype.writeUint16LE = s.prototype.writeUInt16LE = function(A, B, w) {
    return A = +A, B = B >>> 0, w || m(this, A, B, 2, 65535, 0), this[B] = A & 255, this[B + 1] = A >>> 8, B + 2;
  }, s.prototype.writeUint16BE = s.prototype.writeUInt16BE = function(A, B, w) {
    return A = +A, B = B >>> 0, w || m(this, A, B, 2, 65535, 0), this[B] = A >>> 8, this[B + 1] = A & 255, B + 2;
  }, s.prototype.writeUint32LE = s.prototype.writeUInt32LE = function(A, B, w) {
    return A = +A, B = B >>> 0, w || m(this, A, B, 4, 4294967295, 0), this[B + 3] = A >>> 24, this[B + 2] = A >>> 16, this[B + 1] = A >>> 8, this[B] = A & 255, B + 4;
  }, s.prototype.writeUint32BE = s.prototype.writeUInt32BE = function(A, B, w) {
    return A = +A, B = B >>> 0, w || m(this, A, B, 4, 4294967295, 0), this[B] = A >>> 24, this[B + 1] = A >>> 16, this[B + 2] = A >>> 8, this[B + 3] = A & 255, B + 4;
  };
  function LA(E, A, B, w, Q) {
    rA(A, w, Q, E, B, 7);
    let C = Number(A & BigInt(4294967295));
    E[B++] = C, C = C >> 8, E[B++] = C, C = C >> 8, E[B++] = C, C = C >> 8, E[B++] = C;
    let I = Number(A >> BigInt(32) & BigInt(4294967295));
    return E[B++] = I, I = I >> 8, E[B++] = I, I = I >> 8, E[B++] = I, I = I >> 8, E[B++] = I, B;
  }
  function RA(E, A, B, w, Q) {
    rA(A, w, Q, E, B, 7);
    let C = Number(A & BigInt(4294967295));
    E[B + 7] = C, C = C >> 8, E[B + 6] = C, C = C >> 8, E[B + 5] = C, C = C >> 8, E[B + 4] = C;
    let I = Number(A >> BigInt(32) & BigInt(4294967295));
    return E[B + 3] = I, I = I >> 8, E[B + 2] = I, I = I >> 8, E[B + 1] = I, I = I >> 8, E[B] = I, B + 8;
  }
  s.prototype.writeBigUInt64LE = f(function(A, B = 0) {
    return LA(this, A, B, BigInt(0), BigInt("0xffffffffffffffff"));
  }), s.prototype.writeBigUInt64BE = f(function(A, B = 0) {
    return RA(this, A, B, BigInt(0), BigInt("0xffffffffffffffff"));
  }), s.prototype.writeIntLE = function(A, B, w, Q) {
    if (A = +A, B = B >>> 0, !Q) {
      const r = Math.pow(2, 8 * w - 1);
      m(this, A, B, w, r - 1, -r);
    }
    let C = 0, I = 1, x = 0;
    for (this[B] = A & 255; ++C < w && (I *= 256); )
      A < 0 && x === 0 && this[B + C - 1] !== 0 && (x = 1), this[B + C] = (A / I >> 0) - x & 255;
    return B + w;
  }, s.prototype.writeIntBE = function(A, B, w, Q) {
    if (A = +A, B = B >>> 0, !Q) {
      const r = Math.pow(2, 8 * w - 1);
      m(this, A, B, w, r - 1, -r);
    }
    let C = w - 1, I = 1, x = 0;
    for (this[B + C] = A & 255; --C >= 0 && (I *= 256); )
      A < 0 && x === 0 && this[B + C + 1] !== 0 && (x = 1), this[B + C] = (A / I >> 0) - x & 255;
    return B + w;
  }, s.prototype.writeInt8 = function(A, B, w) {
    return A = +A, B = B >>> 0, w || m(this, A, B, 1, 127, -128), A < 0 && (A = 255 + A + 1), this[B] = A & 255, B + 1;
  }, s.prototype.writeInt16LE = function(A, B, w) {
    return A = +A, B = B >>> 0, w || m(this, A, B, 2, 32767, -32768), this[B] = A & 255, this[B + 1] = A >>> 8, B + 2;
  }, s.prototype.writeInt16BE = function(A, B, w) {
    return A = +A, B = B >>> 0, w || m(this, A, B, 2, 32767, -32768), this[B] = A >>> 8, this[B + 1] = A & 255, B + 2;
  }, s.prototype.writeInt32LE = function(A, B, w) {
    return A = +A, B = B >>> 0, w || m(this, A, B, 4, 2147483647, -2147483648), this[B] = A & 255, this[B + 1] = A >>> 8, this[B + 2] = A >>> 16, this[B + 3] = A >>> 24, B + 4;
  }, s.prototype.writeInt32BE = function(A, B, w) {
    return A = +A, B = B >>> 0, w || m(this, A, B, 4, 2147483647, -2147483648), A < 0 && (A = 4294967295 + A + 1), this[B] = A >>> 24, this[B + 1] = A >>> 16, this[B + 2] = A >>> 8, this[B + 3] = A & 255, B + 4;
  }, s.prototype.writeBigInt64LE = f(function(A, B = 0) {
    return LA(this, A, B, -BigInt("0x8000000000000000"), BigInt("0x7fffffffffffffff"));
  }), s.prototype.writeBigInt64BE = f(function(A, B = 0) {
    return RA(this, A, B, -BigInt("0x8000000000000000"), BigInt("0x7fffffffffffffff"));
  });
  function xA(E, A, B, w, Q, C) {
    if (B + w > E.length) throw new RangeError("Index out of range");
    if (B < 0) throw new RangeError("Index out of range");
  }
  function bA(E, A, B, w, Q) {
    return A = +A, B = B >>> 0, Q || xA(E, A, B, 4), D.write(E, A, B, w, 23, 4), B + 4;
  }
  s.prototype.writeFloatLE = function(A, B, w) {
    return bA(this, A, B, !0, w);
  }, s.prototype.writeFloatBE = function(A, B, w) {
    return bA(this, A, B, !1, w);
  };
  function GA(E, A, B, w, Q) {
    return A = +A, B = B >>> 0, Q || xA(E, A, B, 8), D.write(E, A, B, w, 52, 8), B + 8;
  }
  s.prototype.writeDoubleLE = function(A, B, w) {
    return GA(this, A, B, !0, w);
  }, s.prototype.writeDoubleBE = function(A, B, w) {
    return GA(this, A, B, !1, w);
  }, s.prototype.copy = function(A, B, w, Q) {
    if (!s.isBuffer(A)) throw new TypeError("argument should be a Buffer");
    if (w || (w = 0), !Q && Q !== 0 && (Q = this.length), B >= A.length && (B = A.length), B || (B = 0), Q > 0 && Q < w && (Q = w), Q === w || A.length === 0 || this.length === 0) return 0;
    if (B < 0)
      throw new RangeError("targetStart out of bounds");
    if (w < 0 || w >= this.length) throw new RangeError("Index out of range");
    if (Q < 0) throw new RangeError("sourceEnd out of bounds");
    Q > this.length && (Q = this.length), A.length - B < Q - w && (Q = A.length - B + w);
    const C = Q - w;
    return this === A && typeof Uint8Array.prototype.copyWithin == "function" ? this.copyWithin(B, w, Q) : Uint8Array.prototype.set.call(
      A,
      this.subarray(w, Q),
      B
    ), C;
  }, s.prototype.fill = function(A, B, w, Q) {
    if (typeof A == "string") {
      if (typeof B == "string" ? (Q = B, B = 0, w = this.length) : typeof w == "string" && (Q = w, w = this.length), Q !== void 0 && typeof Q != "string")
        throw new TypeError("encoding must be a string");
      if (typeof Q == "string" && !s.isEncoding(Q))
        throw new TypeError("Unknown encoding: " + Q);
      if (A.length === 1) {
        const I = A.charCodeAt(0);
        (Q === "utf8" && I < 128 || Q === "latin1") && (A = I);
      }
    } else typeof A == "number" ? A = A & 255 : typeof A == "boolean" && (A = Number(A));
    if (B < 0 || this.length < B || this.length < w)
      throw new RangeError("Out of range index");
    if (w <= B)
      return this;
    B = B >>> 0, w = w === void 0 ? this.length : w >>> 0, A || (A = 0);
    let C;
    if (typeof A == "number")
      for (C = B; C < w; ++C)
        this[C] = A;
    else {
      const I = s.isBuffer(A) ? A : s.from(A, Q), x = I.length;
      if (x === 0)
        throw new TypeError('The value "' + A + '" is invalid for argument "value"');
      for (C = 0; C < w - B; ++C)
        this[C + B] = I[C % x];
    }
    return this;
  };
  const N = {};
  function _(E, A, B) {
    N[E] = class extends B {
      constructor() {
        super(), Object.defineProperty(this, "message", {
          value: A.apply(this, arguments),
          writable: !0,
          configurable: !0
        }), this.name = `${this.name} [${E}]`, this.stack, delete this.name;
      }
      get code() {
        return E;
      }
      set code(Q) {
        Object.defineProperty(this, "code", {
          configurable: !0,
          enumerable: !0,
          value: Q,
          writable: !0
        });
      }
      toString() {
        return `${this.name} [${E}]: ${this.message}`;
      }
    };
  }
  _(
    "ERR_BUFFER_OUT_OF_BOUNDS",
    function(E) {
      return E ? `${E} is outside of buffer bounds` : "Attempt to access memory outside buffer bounds";
    },
    RangeError
  ), _(
    "ERR_INVALID_ARG_TYPE",
    function(E, A) {
      return `The "${E}" argument must be of type number. Received type ${typeof A}`;
    },
    TypeError
  ), _(
    "ERR_OUT_OF_RANGE",
    function(E, A, B) {
      let w = `The value of "${E}" is out of range.`, Q = B;
      return Number.isInteger(B) && Math.abs(B) > 2 ** 32 ? Q = oA(String(B)) : typeof B == "bigint" && (Q = String(B), (B > BigInt(2) ** BigInt(32) || B < -(BigInt(2) ** BigInt(32))) && (Q = oA(Q)), Q += "n"), w += ` It must be ${A}. Received ${Q}`, w;
    },
    RangeError
  );
  function oA(E) {
    let A = "", B = E.length;
    const w = E[0] === "-" ? 1 : 0;
    for (; B >= w + 4; B -= 3)
      A = `_${E.slice(B - 3, B)}${A}`;
    return `${E.slice(0, B)}${A}`;
  }
  function OA(E, A, B) {
    J(A, "offset"), (E[A] === void 0 || E[A + B] === void 0) && d(A, E.length - (B + 1));
  }
  function rA(E, A, B, w, Q, C) {
    if (E > B || E < A) {
      const I = typeof A == "bigint" ? "n" : "";
      let x;
      throw A === 0 || A === BigInt(0) ? x = `>= 0${I} and < 2${I} ** ${(C + 1) * 8}${I}` : x = `>= -(2${I} ** ${(C + 1) * 8 - 1}${I}) and < 2 ** ${(C + 1) * 8 - 1}${I}`, new N.ERR_OUT_OF_RANGE("value", x, E);
    }
    OA(w, Q, C);
  }
  function J(E, A) {
    if (typeof E != "number")
      throw new N.ERR_INVALID_ARG_TYPE(A, "number", E);
  }
  function d(E, A, B) {
    throw Math.floor(E) !== E ? (J(E, B), new N.ERR_OUT_OF_RANGE("offset", "an integer", E)) : A < 0 ? new N.ERR_BUFFER_OUT_OF_BOUNDS() : new N.ERR_OUT_OF_RANGE(
      "offset",
      `>= 0 and <= ${A}`,
      E
    );
  }
  const zA = /[^+/0-9A-Za-z-_]/g;
  function pA(E) {
    if (E = E.split("=")[0], E = E.trim().replace(zA, ""), E.length < 2) return "";
    for (; E.length % 4 !== 0; )
      E = E + "=";
    return E;
  }
  function $(E, A) {
    A = A || 1 / 0;
    let B;
    const w = E.length;
    let Q = null;
    const C = [];
    for (let I = 0; I < w; ++I) {
      if (B = E.charCodeAt(I), B > 55295 && B < 57344) {
        if (!Q) {
          if (B > 56319) {
            (A -= 3) > -1 && C.push(239, 191, 189);
            continue;
          } else if (I + 1 === w) {
            (A -= 3) > -1 && C.push(239, 191, 189);
            continue;
          }
          Q = B;
          continue;
        }
        if (B < 56320) {
          (A -= 3) > -1 && C.push(239, 191, 189), Q = B;
          continue;
        }
        B = (Q - 55296 << 10 | B - 56320) + 65536;
      } else Q && (A -= 3) > -1 && C.push(239, 191, 189);
      if (Q = null, B < 128) {
        if ((A -= 1) < 0) break;
        C.push(B);
      } else if (B < 2048) {
        if ((A -= 2) < 0) break;
        C.push(
          B >> 6 | 192,
          B & 63 | 128
        );
      } else if (B < 65536) {
        if ((A -= 3) < 0) break;
        C.push(
          B >> 12 | 224,
          B >> 6 & 63 | 128,
          B & 63 | 128
        );
      } else if (B < 1114112) {
        if ((A -= 4) < 0) break;
        C.push(
          B >> 18 | 240,
          B >> 12 & 63 | 128,
          B >> 6 & 63 | 128,
          B & 63 | 128
        );
      } else
        throw new Error("Invalid code point");
    }
    return C;
  }
  function XA(E) {
    const A = [];
    for (let B = 0; B < E.length; ++B)
      A.push(E.charCodeAt(B) & 255);
    return A;
  }
  function qA(E, A) {
    let B, w, Q;
    const C = [];
    for (let I = 0; I < E.length && !((A -= 2) < 0); ++I)
      B = E.charCodeAt(I), w = B >> 8, Q = B % 256, C.push(Q), C.push(w);
    return C;
  }
  function tA(E) {
    return i.toByteArray(pA(E));
  }
  function z(E, A, B, w) {
    let Q;
    for (Q = 0; Q < w && !(Q + B >= A.length || Q >= E.length); ++Q)
      A[Q + B] = E[Q];
    return Q;
  }
  function v(E, A) {
    return E instanceof A || E != null && E.constructor != null && E.constructor.name != null && E.constructor.name === A.name;
  }
  function AA(E) {
    return E !== E;
  }
  const _A = function() {
    const E = "0123456789abcdef", A = new Array(256);
    for (let B = 0; B < 16; ++B) {
      const w = B * 16;
      for (let Q = 0; Q < 16; ++Q)
        A[w + Q] = E[B] + E[Q];
    }
    return A;
  }();
  function f(E) {
    return typeof BigInt > "u" ? $A : E;
  }
  function $A() {
    throw new Error("BigInt not supported");
  }
})(QA);
const xB = ["en", "zh", "ja", "ko", "ar", "th", "pl", "it", "de", "es", "fr"], eA = EA(xB);
U({
  // -----------------used in ui-----------------
  cancel: F(),
  close: F(),
  set: F(),
  clear: F(),
  field: F(),
  fieldName: F(),
  align: F(),
  width: F(),
  opacity: F(),
  height: F(),
  rotate: F(),
  edit: F(),
  required: F(),
  editable: F(),
  plsInputName: F(),
  fieldMustUniq: F(),
  notUniq: F(),
  noKeyName: F(),
  fieldsList: F(),
  editField: F(),
  type: F(),
  errorOccurred: F(),
  errorBulkUpdateFieldName: F(),
  commitBulkUpdateFieldName: F(),
  bulkUpdateFieldName: F(),
  addPageAfter: F(),
  removePage: F(),
  removePageConfirm: F(),
  // --------------------validation-------------------
  "validation.uniqueName": F(),
  "validation.hexColor": F(),
  "validation.dateTimeFormat": F(),
  "validation.outOfBounds": F(),
  // -----------------used in schemas-----------------
  "schemas.color": F(),
  "schemas.borderWidth": F(),
  "schemas.borderColor": F(),
  "schemas.backgroundColor": F(),
  "schemas.textColor": F(),
  "schemas.bgColor": F(),
  "schemas.horizontal": F(),
  "schemas.vertical": F(),
  "schemas.left": F(),
  "schemas.center": F(),
  "schemas.right": F(),
  "schemas.top": F(),
  "schemas.middle": F(),
  "schemas.bottom": F(),
  "schemas.padding": F(),
  "schemas.text.fontName": F(),
  "schemas.text.size": F(),
  "schemas.text.spacing": F(),
  "schemas.text.textAlign": F(),
  "schemas.text.verticalAlign": F(),
  "schemas.text.lineHeight": F(),
  "schemas.text.min": F(),
  "schemas.text.max": F(),
  "schemas.text.fit": F(),
  "schemas.text.dynamicFontSize": F(),
  "schemas.text.format": F(),
  "schemas.radius": F(),
  "schemas.mvt.typingInstructions": F(),
  "schemas.mvt.sampleField": F(),
  "schemas.mvt.variablesSampleData": F(),
  "schemas.barcodes.barColor": F(),
  "schemas.barcodes.includetext": F(),
  "schemas.table.alternateBackgroundColor": F(),
  "schemas.table.tableStyle": F(),
  "schemas.table.showHead": F(),
  "schemas.table.repeatHead": F(),
  "schemas.table.headStyle": F(),
  "schemas.table.bodyStyle": F(),
  "schemas.table.columnStyle": F(),
  "schemas.date.format": F(),
  "schemas.date.locale": F(),
  "schemas.select.options": F(),
  "schemas.select.optionPlaceholder": F(),
  "schemas.radioGroup.groupName": F()
});
EA(["viewer", "form", "designer"]);
const bB = EA(["rgb", "cmyk"]).optional();
U({ height: W(), width: W() });
const O = U({
  name: F(),
  type: F(),
  content: F().optional(),
  position: U({ x: W(), y: W() }),
  width: W(),
  height: W(),
  rotate: W().optional(),
  opacity: W().optional(),
  readOnly: V().optional(),
  required: V().optional(),
  __bodyRange: U({ start: W(), end: W().optional() }).optional(),
  __isSplit: V().optional()
}).passthrough(), GB = U({ id: F() });
O.merge(GB);
const hA = l().refine((g) => g instanceof ArrayBuffer), mA = l().refine((g) => g instanceof Uint8Array && g.buffer instanceof ArrayBuffer), UA = U({
  width: W(),
  height: W(),
  padding: AB([W(), W(), W(), W()]),
  staticSchema: T(O).optional()
}), oB = wA([F(), hA, mA]), rB = wA([oB, UA]);
T(y(F(), O));
const tB = T(T(O)), jA = U({
  schemas: tB,
  basePdf: rB,
  pdfmeVersion: F().optional()
}).passthrough(), CA = T(y(F(), l())).min(1), cB = y(F(), U({
  data: wA([F(), hA, mA]),
  fallback: V().optional(),
  subset: V().optional()
})), WB = U({
  ui: l(),
  pdf: l(),
  propPanel: U({
    schema: WA(),
    widgets: y(F(), l()).optional(),
    defaultSchema: O
  }),
  icon: F().optional()
}).passthrough(), gA = U({ font: cB.optional() }).passthrough(), PA = U({
  template: jA,
  options: gA.optional(),
  plugins: y(F(), WB).optional()
}), nB = gA.extend({
  colorType: bB,
  author: F().optional(),
  creationDate: cA().optional(),
  creator: F().optional(),
  keywords: T(F()).optional(),
  lang: eA.optional(),
  modificationDate: cA().optional(),
  producer: F().optional(),
  subject: F().optional(),
  title: F().optional()
}), eB = PA.extend({
  inputs: CA,
  options: nB.optional()
}).strict(), vA = gA.extend({
  lang: eA.optional(),
  labels: y(F(), F()).optional(),
  theme: y(F(), WA()).optional(),
  icons: y(F(), F()).optional(),
  requiredByDefault: V().optional(),
  maxZoom: W().optional(),
  sidebarOpen: V().optional(),
  zoomLevel: W().optional()
}), hB = l().refine((g) => g instanceof HTMLElement), IA = PA.extend({
  domContainer: hB,
  options: vA.optional()
}), mB = IA.extend({ inputs: CA }).strict(), UB = IA.extend({}).strict(), NB = structuredClone, HA = (g) => Array.from(new Set(g)), JB = (g) => {
  const i = "", D = Object.entries(g).reduce((M, R) => {
    const [L, Y] = R;
    return !M && Y.fallback ? L : M;
  }, i);
  if (D === i)
    throw Error("[@pdfme/common] fallback flag is not found in font. true fallback flag must be only one.");
  return D;
}, aB = () => ({
  [CB]: { data: vB(gB), fallback: !0 }
}), TB = (g) => parseFloat(String(g)) * QB, ZB = (g) => g * wB, kB = (g) => g * EB, dB = (g) => parseFloat(String(g)) * 0.26458333333333, jB = (g) => new Promise((i, D) => {
  const M = new FileReader();
  M.onloadend = () => {
    M.result.startsWith("data:application/pdf;") ? i(M.result) : D(Error("[@pdfme/common] template.basePdf must be pdf data."));
  }, M.readAsDataURL(g);
}), OB = (g) => /^#(?:[A-Fa-f0-9]{3,4}|[A-Fa-f0-9]{6}|[A-Fa-f0-9]{8})$/i.test(g), X = (g) => {
  g.schemas && Array.isArray(g.schemas) && g.schemas.length > 0 && !Array.isArray(g.schemas[0]) && (g.schemas = g.schemas.map((i) => Object.entries(i).map(([D, M]) => ({
    ...M,
    name: D
  }))));
}, zB = (g) => {
  X(g);
  const i = {};
  return g.schemas.forEach((D) => {
    D.forEach((M) => {
      M.readOnly || (i[M.name] = M.content || "");
    });
  }), [i];
}, pB = async (g) => {
  if (typeof g == "string" && !g.startsWith("data:application/pdf;") && typeof window < "u") {
    const M = await (await fetch(g)).blob();
    return jB(M);
  }
  if (typeof g == "string")
    return g;
  const i = g instanceof Uint8Array ? g : new Uint8Array(g);
  return "data:application/pdf;base64," + QA.Buffer.from(i).toString("base64");
}, XB = (g) => UA.safeParse(g).success, PB = (g) => QA.Buffer.from(g, "base64").toString("binary"), vB = (g) => {
  const i = g.split(";base64,")[1] ? g.split(";base64,")[1] : g, D = PB(i), M = new Uint8Array(D.length);
  for (let R = 0; R < D.length; R += 1)
    M[R] = D.charCodeAt(R);
  return M;
}, HB = (g) => HA(g.map((i) => i.map((D) => D.fontName ?? "")).reduce((i, D) => i.concat(D), []).filter(Boolean)), SB = (g) => {
  const { font: i, template: { schemas: D } } = g, R = Object.values(i).reduce((s, b) => b.fallback ? s + 1 : s, 0);
  if (R === 0)
    throw Error(`[@pdfme/common] fallback flag is not found in font. true fallback flag must be only one.
Check this document: https://pdfme.com/docs/custom-fonts#about-font-type`);
  if (R > 1)
    throw Error(`[@pdfme/common] ${R} fallback flags found in font. true fallback flag must be only one.
Check this document: https://pdfme.com/docs/custom-fonts#about-font-type`);
  const L = HB(D), Y = Object.keys(i);
  if (L.some((s) => !Y.includes(s)))
    throw Error(`[@pdfme/common] ${L.filter((s) => !Y.includes(s)).join()} of template.schemas is not found in font.
Check this document: https://pdfme.com/docs/custom-fonts`);
}, fB = (g) => {
  const { plugins: i, template: { schemas: D } } = g, M = HA(D.map((L) => L.map((Y) => Y.type)).flat()), R = Object.values(i).map((L) => L ? L.propPanel.defaultSchema.type : void 0);
  if (M.some((L) => !R.includes(L)))
    throw Error(`[@pdfme/common] ${M.filter((L) => !R.includes(L)).join()} of template.schemas is not found in plugins.`);
}, K = (g, i) => {
  try {
    i.parse(g);
  } catch (D) {
    if (D instanceof BB) {
      const M = D.issues.map((R) => `ERROR POSITION: ${R.path.join(".")}
ERROR MESSAGE: ${R.message}
--------------------------`);
      throw Error(`[@pdfme/common] Invalid argument:
--------------------------
${M.join(`
`)}`);
    } else
      throw Error(`[@pdfme/common] Unexpected parsing error: ${D instanceof Error ? D.message : String(D)}`);
  }
  if (g && typeof g == "object" && "template" in g && "options" in g) {
    const { template: D, options: M } = g;
    M && M.font && SB({ font: M.font, template: D });
  }
  if (g && typeof g == "object" && "template" in g && "plugins" in g) {
    const { template: D, plugins: M } = g;
    M && fB({ plugins: M, template: D });
  }
}, qB = (g) => K(g, CA), _B = (g) => K(g, vA), $B = (g) => K(g, mB), AE = (g) => K(g, UB), BE = (g) => {
  typeof g == "object" && g !== null && "template" in g && X(g.template), K(g, IA);
}, EE = (g) => {
  X(g), K(g, jA);
}, wE = (g) => {
  typeof g == "object" && g !== null && "template" in g && X(g.template), K(g, eB);
};
export {
  VB as B,
  sB as C,
  CB as D,
  QB as M,
  wB as P,
  uB as Z,
  lB as a,
  EB as b,
  vB as c,
  AE as d,
  SB as e,
  wE as f,
  qB as g,
  $B as h,
  EE as i,
  _B as j,
  BE as k,
  NB as l,
  pB as m,
  aB as n,
  JB as o,
  zB as p,
  XB as q,
  OB as r,
  TB as s,
  ZB as t,
  kB as u,
  dB as v,
  p as w,
  QA as x
};
